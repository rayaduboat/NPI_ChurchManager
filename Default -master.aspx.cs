        ...
        try
        {
            ...
            PiwcMember foundMember = (from match in db.Members
                                      ...
                                      {   Config=db.Configurations.Select(i=>i),
                                          ...
                                          AssignedBranches = db.AssignedBranches.AsEnumerable()
                                          .Where(p=>p.MemberID==match.MemberID)
                                          .Select(p => new {
                                              BranchID=p.AssignedBranchID,
                                              BranchName=db.Branches.Where(t=>t.ID==p.AssignedBranchID).Select(t=>t.Name).FirstOrDefault()
                                          }), 
