...
$rab.Church.LoadBranchNamesInSelectControls = function (dropdownArray) {
    //loads church branches into dropdown control supplied
...
$rab.Church.GetDistrictAndAreas = function (func) {
    //loads church branches into dropdown control supplied
    //func handler handler to passed read branches
    var branches = "";
    try {
        ...
        handler = function (jsonData) {
            try {
                ...
                if (JSON.parse(jsonData).message == "failed")
                    throw new $rab.Exception('failed to get district and area names');
...
$rab.Security.Church.LoadBranches = function (branchName) {
    try {
        ...
        if (user.LoginType == "SuperUser" || user.LoginType == "FinanceController") {
            ...
            new $rab.AssignedBranches().LoadBranches(user, branchName); return;
        }
...
$rab.Security.LockDown = function () {
    ...
    this.DisplayLoggedInUser = function () {
        $("#aloginUser").html("Welcome " + $rab.Login.User.FirstName + " " + $rab.Login.User.LastName + " [" + $rab.Login.User.BranchName + "]");
    }
    ...
    this.HideMenuDropdowns = function () {
        try {
            ...
            if ($rab.Login.User.Permissionset.filter(function (val) { return val == 'Advanced search' }).length == 0) $("a[href='Search.aspx']").hide();
...
$rab.Customerization = function () {
    this.theme = function () {
        $('.modal-title').html('Select theme for your table reports');
        $('#defaultBody').empty();
        var html = '<ul class= "list-group" ><li onclick="new $rab.Customerization().select(2)" class="list-group-item">';
        ...
        html = html + '</li><li onclick="new $rab.Customerization().select(1)" class="list-group-item">';
        ...
        html = html + '</li>';
        html = html + '<li onclick="new $rab.Customerization().select(3)" class="list-group-item">';
    ...
    this.select = function (id) {
        try {
            ...
            var theme = new $rab.Server.Dispatcher();
            theme.obj = { memberID: $rab.Login.User.MemberID, themeID: id };
            theme.handler = function (obj) {
                if (obj == 'success') {
                    //update localstorage with themeID
                    $rab.Login.User.ThemeID = id;
                    localStorage.setItem('user', JSON.stringify($rab.Login.User));
