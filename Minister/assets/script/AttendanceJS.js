...
$rab.Attendance.Add = function (button) {
    var covidCheck = document.getElementById('checkCovidAttend');
    if (covidCheck.checked) {
        //this is COVID provision for Templerature
    //=================================================
    var tempVal = $('#txtTemperature').val();
        if (tempVal == "") {
            ...
            try {
                ...
                handler = function (jsonmessage) {
                    try {
                        ...
                        //Deal with Temperature here
                        //===============================
                        new $rab.Attendance.addMemberTemperature(tempVal, mem.MemberID);
                    } catch (e) {
                        alert(e.message);
                    }
...
$rab.Attendance.addMemberTemperature = function (temp, memId) {
    var url, obj, handler;
    url = "Attendance.aspx/AddTemperature";
    obj = { temperature: temp, memberId: memId };
...
$rab.Attendance.LoadMembers = function (branchName) {
    ...
    //load members on buttons to mark attendance
            ...
            try {
                ...
                button.onclick = function () {
                    try {
                        ...
                        //check if attendance is WITH or WITHOUT COVID temperature
                        //===========================================================
                        var covidCheck = document.getElementById('checkCovidAttend');
                        var markText = "Mark <b>" + activity + "</b> attendance for <kbd>" + this.FullName + "</kbd>, click Mark to mark or Close to cancel"
                        if (covidCheck.checked) {
                            $("#buttonModal").click();
                            $("#modalContent").html(markText);
                        } else {
                            $("#buttonModalNT").click();
                            $("#modalContentNT").html(markText);
                        }
                        //$("#buttonModal").click(); ===PREVIOUS

                        //$("#modalContent").html(markText);//("Mark <b>" + activity + "</b> attendance for <kbd>" + this.FullName + "</kbd>, click Mark to mark or Close to cancel");
                        //$("#modalContentNT").html(markText);//("Mark <b>" + activity + "</b> attendance for <kbd>" + this.FullName + "</kbd>, click Mark to mark or Close to cancel");
                        $("#txtTemperature").val("");
...
$rab.Attendance.Reports = function () {
    
   
}
//var getParamToServer=function(url, obj, handler) {
//        $.ajax({
//            type: "POST",
//            url: url,
